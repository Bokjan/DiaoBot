...
static void CurlHttpRequest(long &response_code, string &retbody, const string &url,
  ...
  if (!proxy) {
    ...
    if (origin_no_proxy != nullptr) env_no_proxy = origin_no_proxy;
  ...
  if (data != nullptr) {
    ...
    if (data_len != 0) curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, data_len);
  }
  string header;
...
int BotEngine::SendMessage(const WeworkMessage &message) {
  ...
  if (response.ResponseCode != 200) return static_cast<int>(-response.ResponseCode);
