...
void WeworkMessage::SetChatID(const string &chatid) { this->ChatID = chatid; }
...
void WeworkTextMessage::AddMentioned(const string &m) { PImpl->MentionedList.emplace_back(m); }
...
std::shared_ptr<WeworkMarkdownMessage::ButtonAction>
WeworkMarkdownMessage::Attachment::AddButtonAction(void) {
...
void WeworkImageMessage::SetRawImage(const string &buf) { RawImageBuffer = buf; }
...
string WeworkImageMessage::GetJson(void) const {
  ...
  w.StartObject();
  w.Key("base64");
  w.String(base64_encode(reinterpret_cast<const unsigned char *>(RawImageBuffer.data()),
